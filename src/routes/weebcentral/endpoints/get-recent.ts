...
endpoint.openapi(route, async (c) => {
	...
	try {
		...
		const url = `${BASE_URL}/search/data?limit=${_count}&offset=${_count * _page}&sort=Latest+Updates&order=Descending&official=Any&anime=Any&adult=Any&display_mode=Full+Display`;
